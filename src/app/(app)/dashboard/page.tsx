 ...
 const loadChecklist = useCallback(async (scope: string) => {
  ...
  try {
    ...
    if (err.response?.status === 404) {
      try {
        ...
        // 3️⃣ Create seeded checklist for user
        const createRes = await axios.post(ROUTES.CREATE_CHECKLIST, {
          scope,
          categories: template.categories, // 👈 assuming TemplateModel has this shape
        });
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      ...
      toast.success('We found additional checks to consider.');
    } catch (e: any) {
